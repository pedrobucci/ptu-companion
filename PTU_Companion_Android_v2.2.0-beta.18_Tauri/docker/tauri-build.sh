...
copy_installable_apk() {
  local source_apk="$1"
  local output="$ANDROID_OUT/PTU-Companion-v2.2.0-beta.18-installable.apk"
  cp -f "$source_apk" "$output"
  verify_apk "$output" | tee "$ANDROID_OUT/PTU-Companion-v2.2.0-beta.18-installable-info.txt"
...
build_android_arm64_apk() {
  ...
  local output="$ANDROID_OUT/PTU-Companion-v2.2.0-beta.18-arm64.apk"
  cp -f "$apk" "$output"
...
build_android_arm64_release_apk() {
  ...
  local aligned_apk="$ANDROID_OUT/.PTU-Companion-v2.2.0-beta.18-arm64-release-aligned.apk"
  local output="$ANDROID_OUT/PTU-Companion-v2.2.0-beta.18-arm64-release.apk"
  ...
  verify_apk "$output" | tee "$ANDROID_OUT/PTU-Companion-v2.2.0-beta.18-arm64-release-info.txt"
...
inspect_apk() {
  local apk="${2:-$ANDROID_OUT/PTU-Companion-v2.2.0-beta.18-installable.apk}"
  verify_apk "$apk"
}
