/*
 * PTU Companion — Beta v2.1.0-beta.5
...
let androidContentPackState={packs:[],loading:false,error:null};
...
async function loadCatalogStatus(){
  ...
  try{
    const [statusRes,rulesetsRes,guidanceRes]=await Promise.all([fetch('/api/definitions/status',{cache:'no-store'}),fetch('/api/rulesets',{cache:'no-store'}),fetch('/api/pokemon/evolution-guidance',{cache:'no-store'})]);
...
function migrateState(input){
  ...
  for(const p of data.pokemon){ p.details=p.details&&typeof p.details==='object'?p.details:{}; p.details.gender=normalizePokemonGender(p.details.gender??p.gender??p.sex??'None'); }
...
function openMobileMore(){
  modal(`<h2>More</h2><div class="mobile-more-grid">
    <button class="btn btn-ghost" onclick="closeModal();route('trainer')">🪪 Trainer</button>
    ...
    <button class="btn btn-ghost" onclick="closeModal();route('library')">📚 Pokédex & Rules</button>
    <button class="btn btn-ghost" onclick="closeModal();route('npcs')">📓 NPCs</button>
    <button class="btn btn-ghost" onclick="closeModal();route('levelup')">✦ Level Up</button>
    <button class="btn btn-ghost" onclick="closeModal();openContentPackManager()">📦 Content Packs</button>
    <button class="btn btn-ghost" onclick="closeModal();openSaveTools()">⇄ Save Tools</button>
    <button class="btn btn-ghost" onclick="closeModal();openGlobalActions()">⚙ Settings</button>
  </div>`);
}
function mobileBottomNav(){
  if(!window.PTU_ANDROID_BUILD)return '';
  const items=[['dashboard','⌂','Home'],['creature','🐾','Creatures'],['rosters','◉','Rosters'],['inventory','🎒','Items']];
  const primaryActive=items.some(n=>state.ui.screen===n[0]); return `<nav class="mobile-bottom-nav">${items.map(n=>`<button class="${state.ui.screen===n[0]?'active':''}" onclick="route('${n[0]}')"><span>${n[1]}</span><small>${n[2]}</small></button>`).join('')}<button class="${primaryActive?'':'active'}" onclick="openMobileMore()"><span>☰</span><small>More</small></button></nav>`;
}
function shell(content){
  ...
  const platformLabel=window.PTU_ANDROID_BUILD?'ANDROID BETA v2.2.0-beta.18':'BETA v2.1.0';
...
function dashboard(){
  ...
  return `<div class="page dashboard-page">${heading('ACTIVE CAMPAIGN','Trainer Command Center','Your campaign state now persists between sessions.','<button class="btn btn-primary" onclick="route(\'levelup\')">✦ Guided Level-Up</button>')}
...
function trainerScreen(){
  ...
  return `<div class="page trainer-page">${head}${tabs}${body}</div>`;
}
...
function rostersScreen(){
  ...
  return `<div class="page">${heading('ROSTERS','Team Organization','Multiple rosters can be active at once. A Pokémon may belong to several rosters.','<button class="btn btn-primary" onclick="createRoster()">＋ Create Roster</button>')}
  <div class="roster-selector">${state.rosters.map(x=>`<button class="${x.id===r.id?'active':''}" style="--accent:${x.color}" onclick="selectRoster('${x.id}')"><strong>${esc(x.name)}</strong>${chip(esc(x.role),x.role==='COMBAT'?'chip-green':x.role==='COMPANY'?'chip-blue':'chip-yellow')}<b>${rosterMembers(x.id).length}/${x.maxMembers}</b></button>`).join('')}</div>
...
function storageScreen(){
  ...
  </div><div class="flow-note"><strong>Implemented invariant:</strong> Storage resets current HP to maximum, resets combat stages, preserves permanent data, and returns a Held Item to the Backpack.</div></div>`;
}
...
function renderBackpackItemPicker(){
  ...
  const filtered=backpackItemPickerFiltered();
  modal(`<div class="catalog-add-toolbar"><input id="backpack-item-search" value="${esc(itemCatalogState.query)}" oninput="filterBackpackItemPicker(this.value)" placeholder="Search item, category, effect, Trainer, Head, Accessory…" autocomplete="off"/><span id="backpack-item-search-count">${filtered.length} / ${itemCatalogState.items.length}</span></div><div id="backpack-item-search-results" class="catalog-add-list">${backpackItemPickerRows(filtered)}</div>`,{title:'Add Game Item',subtitle:`${itemCatalogState.items.length} catalogued items in the active Ruleset. Usability and equipment slots are shown before adding.`});
  requestAnimationFrame(()=>document.getElementById('backpack-item-search')?.focus());
}
function filterBackpackItemPicker(value){
  itemCatalogState.query=value;
  // Android: update only the result nodes. Rebuilding the whole modal replaces the focused
  // <input>, which makes Samsung Keyboard close/reopen for every character.
  const filtered=backpackItemPickerFiltered();
  const list=document.getElementById('backpack-item-search-results');
  const count=document.getElementById('backpack-item-search-count');
  if(list) list.innerHTML=backpackItemPickerRows(filtered);
  if(count) count.textContent=`${filtered.length} / ${itemCatalogState.items.length}`;
}
function addCatalogItemToBackpack(id){
  ...
  persist(); render();
  const input=document.getElementById('backpack-item-search'); if(input) filterBackpackItemPicker(input.value);
}
...
function definitionRow(d){
  ...
  return `<button class="definition-row ${catalogState.selected?.definition?.id===d.id?'selected':''}" onclick="loadDefinitionDetail('${esc(d.id)}')"><div><h3>${esc(d.name)}</h3><div class="definition-row-meta">${tags.join('')}</div><p>${esc(d.effect||definitionSummaryText(d))}</p></div><div class="definition-source"><strong>${esc(d.packName||d.contentPackId)}</strong>${d.sourcePage?`<br/>p. ${d.sourcePage}`:''}</div></button>`;
}
function definitionDetail(){
  ...
  return `<div class="definition-hero"><div><h2>${esc(d.name)}</h2><p>${esc(d.id)} · ${esc(d.kind)}</p></div>${d.needsReview?chip('NEEDS REVIEW','chip-yellow'):chip('RESOLVED','chip-green')}</div>${body}`;
}
function libraryScreen(){
  ...
  const libraryAction=window.PTU_ANDROID_BUILD?`<button class="btn btn-ghost" onclick="openContentPackManager()">📦 Manage Packs</button>`:(catalogState.loading?chip('LOADING','chip-yellow'):chip('SQLITE + RULESET','chip-green'));
  return `<div class="page">${heading('REAL PTU DATA','Pokédex & Rules Library','Resolved definitions now come from the v1.0 PTU seed database instead of UI mock data.',libraryAction)}
...
function pokemonProgressStatRows(pv,evolved){
  ...
  return ['hp','attack','defense','special_attack','special_defense','speed'].map(k=>{
    const label=statDisplayName(k); const before=evolved?pv.natureAdjustedBaseStats?.[k]:(pv.previousStatAllocations?.[k]||0); const add=Number(alloc[k]||0); const final=pv.resolvedFinalStats?.[k]??pv.finalStats?.[k];
    return `<div class="pokemon-progress-stat-row"><span class="pokemon-progress-stat-name"><strong>${esc(label)}</strong>${k==='hp'?'<small>HP exempt from Base Relations</small>':''}</span><em class="pokemon-progress-stat-before">${before??0}</em><div class="pokemon-progress-stepper"><button type="button" aria-label="Remove one ${esc(label)} point" onclick="adjustPokemonProgressStat('${k}',-1,${evolved})" ${add<=0?'disabled':''}>−</button><b>${add}</b><button type="button" aria-label="Add one ${esc(label)} point" onclick="adjustPokemonProgressStat('${k}',1,${evolved})">＋</button></div><strong class="pokemon-progress-stat-final">${final??'—'}</strong></div>`;
  }).join('');
}
function pokemonProgressionScreen(){
  ...
  const statSection=pv?`${pv.evolved?`<div class="flow-note"><strong>Evolution re-Stat:</strong> PTU reapplies the new form's Base Stats and Nature, then redistributes Level + 10 Stat Points. Your campaign rule continues to exempt HP from Base Relations.</div><button class="btn btn-ghost btn-small" onclick="useSuggestedEvolutionRestat()">Use suggested re-Stat</button>`:`<div class="flow-note"><strong>Normal Level-Up:</strong> existing Stat allocation is preserved; distribute only the ${pv.rewards.statPoints} newly earned point${pv.rewards.statPoints===1?'':'s'}.</div>`}<div class="pokemon-progress-stat-head"><span>Stat</span><span>${pv.evolved?'Nature Base':'Existing alloc.'}</span><span>${pv.evolved?'Re-Stat allocation':'New points'}</span><span>Final</span></div><div class="pokemon-progress-stat-list">${pokemonProgressStatRows(pv,pv.evolved)}</div><div class="builder-budget ${pv.evolved?(pv.statBudget.spent===pv.statBudget.required?'ok':'warn'):(pv.statBudget.newPointsSpent===pv.statBudget.newPointsRequired?'ok':'warn')}"><strong>${pv.evolved?`${pv.statBudget.spent}/${pv.statBudget.required} total Stat Points`:`${pv.statBudget.newPointsSpent}/${pv.statBudget.newPointsRequired} new Stat Points`}</strong></div>`:'';
  const abilityRows=pv?pv.abilitySlots.map((slot,i)=>`<label class="ability-choice"><span><strong>${esc(slot.label)}</strong><small>Unlocked Lv. ${slot.unlockLevel}</small></span><select onchange="setPokemonProgressAbility(${i},this.value)"><option value="">Choose…</option>${slot.options.map(a=>`<option value="${esc(a.name)}" ${(f.selectedAbilities||[])[i]===a.name?'selected':''}>${esc(a.name)} · ${esc(a.category)}</option>`).join('')}</select></label>`).join(''):'';
  const newMoveSet=new Set(pv?.newMoveKeys||[]); const existingSet=new Set(pv?.existingMoveKeys||[]);
  const moveRows=pv?(pv.movePool||[]).map(m=>`<label class="builder-move-option ${newMoveSet.has(m.key)?'new-progression-move':''}"><input type="checkbox" ${(f.selectedMoves||[]).includes(m.key)?'checked':''} onchange="togglePokemonProgressMove('${esc(m.key)}')"/><span><strong>${esc(m.name)}</strong><small>${existingSet.has(m.key)?'Already known':esc(m.eligibilityReason||'Newly available')}</small></span><span class="move-source-tags">${existingSet.has(m.key)?chip('KNOWN','chip-blue'):chip(pv.evolved?'EVOLUTION':'NEW','chip-green')}</span></label>`).join(''):'';
...
function render({preserveScroll=true}={}){
  if(window.PTU_ANDROID_BUILD&&state.ui.screen==='editor')state.ui.screen='dashboard';
...
async function downloadJson(data,filename){
  ...
  if(window.PTU_ANDROID_BUILD){
    const dialog=window.__TAURI__?.dialog, fs=window.__TAURI__?.fs;
    if(dialog?.save && fs?.writeTextFile){
      const path=await dialog.save({defaultPath:filename,filters:[{name:'JSON Save',extensions:['json']}]});
      if(!path)return false;
      await fs.writeTextFile(path,text);
      return true;
    }
    throw new Error('Android native file export is unavailable. Update/reinstall the current PTU Companion build.');
  }
  const blob=new Blob([text],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=filename; document.body.appendChild(a); a.click(); setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove();},100); return true;
}
function semanticVersionParts(v){ return String(v||'0').split(/[^0-9]+/).filter(Boolean).slice(0,4).map(Number); }
function versionAtLeast(actual,minimum){ const a=semanticVersionParts(actual),b=semanticVersionParts(minimum),n=Math.max(a.length,b.length);for(let i=0;i<n;i++){const av=a[i]||0,bv=b[i]||0;if(av>bv)return true;if(av<bv)return false;}return true; }
async function currentContentPacks(){
  try{const r=await fetch('/api/content-packs',{cache:'no-store'});const p=await r.json();androidContentPackState.packs=p.packs||[];androidContentPackState.error=null;return androidContentPackState.packs;}
  catch(error){androidContentPackState.error=error.message;return androidContentPackState.packs||[];}
}
function contentPackIdsUsedByState(snapshot){
  const out=new Set();
  const visit=value=>{if(!value)return;if(Array.isArray(value)){value.forEach(visit);return;}if(typeof value!=='object')return;for(const [key,v] of Object.entries(value)){if((key==='contentPackId'||key==='content_pack_id'||key==='speciesContentPackId')&&typeof v==='string'&&v.trim())out.add(v.trim());visit(v);}};
  visit(snapshot); return [...out];
}
async function saveContentDependencies(snapshot){
  const packs=await currentContentPacks(),byId=new Map(packs.map(p=>[String(p.id),p]));
  return contentPackIdsUsedByState(snapshot).map(packId=>({packId,minVersion:String(byId.get(packId)?.version||'0.0.0')})).sort((a,b)=>a.packId.localeCompare(b.packId));
}
async function exportSave(){
  try{
    const snapshot=JSON.parse(JSON.stringify(state)); snapshot.contentDependencies=await saveContentDependencies(snapshot);
    const saved=await downloadJson(snapshot,`ptu-companion-${trainer().name.replace(/\W+/g,'-').toLowerCase()}.json`);
    if(saved)toast(window.PTU_ANDROID_BUILD?'JSON save exported. Choose its destination in the Android file picker.':'Save exported with Content Pack dependencies.');
  }catch(error){toast(`Export failed: ${error.message}`,'error');}
}
async function missingSaveDependencies(data){
  const declared=Array.isArray(data?.contentDependencies)?data.contentDependencies:[]; const inferred=contentPackIdsUsedByState(data).map(packId=>({packId,minVersion:'0'})); const requested=[...new Map([...declared,...inferred].map(dep=>[String(dep.packId||dep.id||''),dep])).values()].filter(dep=>String(dep.packId||dep.id||'')); if(!requested.length)return [];
  const packs=await currentContentPacks(),byId=new Map(packs.map(p=>[String(p.id),p]));
  return requested.filter(dep=>{const id=String(dep.packId||dep.id||'');const installed=byId.get(id);return !installed||!versionAtLeast(installed.version,dep.minVersion||dep.version||'0');}).map(dep=>({packId:String(dep.packId||dep.id||''),minVersion:String(dep.minVersion||dep.version||'0'),installedVersion:String(byId.get(String(dep.packId||dep.id||''))?.version||'not installed')}));
}
async function importSaveFile(file){
  if(!file)return;
  try{
    const data=JSON.parse(await file.text()); if(!data||![1,2].includes(data.version)||!data.trainer||!Array.isArray(data.pokemon)) throw new Error('Unsupported save format');
    const missing=await missingSaveDependencies(data);
    if(missing.length){
      modal(`<div class="builder-validation bad"><strong>Missing Content Packs</strong><span>This save uses definitions that are not installed on this device.</span></div><div class="content-pack-dependency-list">${missing.map(d=>`<div><strong>${esc(d.packId)}</strong><small>Required ${esc(d.minVersion)} · Installed ${esc(d.installedVersion)}</small></div>`).join('')}</div><div class="modal-actions horizontal"><button class="btn btn-ghost" onclick="closeModal()">Cancel</button><button class="btn btn-primary" onclick="closeModal();openSaveTools()">Install .ptucp</button></div>`,{title:'Save requires additional content',subtitle:'Install the required packs, then import the JSON again.',tone:'red'}); return;
    }
    state=migrateState(data); invalidateTrainerReference(); persist(); render(); toast('Save imported.'); if(state.ui.screen==='trainer')setTimeout(()=>loadTrainerReferenceData(true),0);
...
async function importContentPackFile(file){
  if(!file)return;
  if(!String(file.name||'').toLowerCase().endsWith('.ptucp'))return toast('Choose a .ptucp Content Pack.','error');
  try{
    toast(`Validating ${file.name}…`);
    const archiveB64=arrayBufferToBase64(await file.arrayBuffer());
    const response=await fetch('/api/content-packs/import',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({archiveB64,archiveFilename:file.name,enableRulesetId:catalogState.status?.activeRulesetId||'all-provided-material'})});
    const payload=await response.json().catch(()=>({})); if(!response.ok)throw new Error(payload.error||'Content Pack import failed.');
    await currentContentPacks(); catalogState.status=null;catalogState.available=false;catalogState.rows=[];catalogState.selected=null;itemCatalogState={loading:false,error:null,query:'',items:[]};
    await loadCatalogStatus();
    const counts=Object.entries(payload.pack?.counts||{}).map(([k,v])=>`${v} ${k}`).join(' · ');
    const active=payload.pack?.enabled!==false;
    modal(`<div class="builder-validation good"><strong>${esc(payload.pack?.name||payload.pack?.id||'Content Pack')} installed</strong><span>Version ${esc(payload.pack?.version||'')} ${counts?`· ${esc(counts)}`:''}</span></div>${!active?'<div class="flow-note">This pack was updated but remains disabled in the current Ruleset.</div>':''}${(payload.pack?.warnings||[]).length?`<div class="flow-note">${payload.pack.warnings.map(w=>`<div>⚠ ${esc(w)}</div>`).join('')}</div>`:''}<div class="modal-actions"><button class="btn btn-primary full" onclick="closeModal();openContentPackManager()">Manage Content Packs</button></div>`,{title:'Content Pack installed',subtitle:active?'Enabled in the current Ruleset.':'Updated without changing its disabled state.'});
  }catch(error){modal(`<div class="builder-validation bad"><strong>Content Pack was not installed</strong><span>${esc(error.message)}</span></div><div class="modal-actions"><button class="btn btn-primary" onclick="closeModal();openContentPackManager()">Back</button></div>`,{title:'Import .ptucp',tone:'red'});}
}

async function setAndroidContentPackEnabled(packId,enabled){
  const packs=await currentContentPacks(); const pack=packs.find(p=>String(p.id)===String(packId)); if(!pack)return;
  const used=contentPackIdsUsedByState(state).includes(packId);
  if(!enabled){
    const ok=await styledConfirm({title:`Disable ${pack.name||pack.id}?`,message:`<p>The pack stays installed on this device but is removed from the active Ruleset.</p>${used?'<div class="dialog-warning">This Trainer save references definitions from the pack. Existing entries remain, but some rules/lookups can be unavailable until it is enabled again.</div>':''}`,confirmLabel:'Disable Pack',tone:'gold'});
    if(!ok)return;
  }
  try{
    toast(`${enabled?'Enabling':'Disabling'} ${pack.name||pack.id}…`);
    const response=await fetch(`/api/content-packs/${encodeURIComponent(packId)}/enabled`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled,rulesetId:catalogState.status?.activeRulesetId||'all-provided-material'})});
    const payload=await response.json().catch(()=>({})); if(!response.ok)throw new Error(payload.error||'Could not update Content Pack state.');
    await currentContentPacks(); catalogState.status=null;catalogState.available=false;catalogState.rows=[];catalogState.selected=null;itemCatalogState={loading:false,error:null,query:'',items:[]}; await loadCatalogStatus();
    toast(`${pack.name||pack.id} ${enabled?'enabled':'disabled'}.`); await openContentPackManager();
  }catch(error){modal(`<div class="builder-validation bad"><strong>Content Pack was not changed</strong><span>${esc(error.message)}</span></div><div class="modal-actions"><button class="btn btn-primary" onclick="closeModal();openContentPackManager()">Back</button></div>`,{title:'Content Pack',tone:'red'});}
}
async function uninstallAndroidContentPack(packId){
  const packs=await currentContentPacks(); const pack=packs.find(p=>String(p.id)===String(packId)); if(!pack)return;
  if(!pack.removable)return toast('Bundled packs cannot be uninstalled on Android.','error');
  const used=contentPackIdsUsedByState(state).includes(packId);
  const ok=await styledConfirm({title:`Uninstall ${pack.name||pack.id}?`,message:`<p>This removes the imported <code>.ptucp</code> and its definitions from this device.</p>${used?'<div class="dialog-warning">This Trainer save references the pack. Disabling is safer; uninstalling may leave unresolved references until you import it again.</div>':'<div class="dialog-warning">You can reinstall it later by importing the same .ptucp file.</div>'}`,confirmLabel:'Uninstall Pack',danger:true});
  if(!ok)return;
  try{
    const response=await fetch(`/api/content-packs/${encodeURIComponent(packId)}`,{method:'DELETE'}); const payload=await response.json().catch(()=>({})); if(!response.ok)throw new Error(payload.error||'Could not uninstall Content Pack.');
    await currentContentPacks(); catalogState.status=null;catalogState.available=false;catalogState.rows=[];catalogState.selected=null;itemCatalogState={loading:false,error:null,query:'',items:[]}; await loadCatalogStatus();
    toast(`${pack.name||pack.id} uninstalled.`); await openContentPackManager();
  }catch(error){modal(`<div class="builder-validation bad"><strong>Content Pack was not removed</strong><span>${esc(error.message)}</span></div><div class="modal-actions"><button class="btn btn-primary" onclick="closeModal();openContentPackManager()">Back</button></div>`,{title:'Uninstall Content Pack',tone:'red'});}
}


async function openContentPackManager(){
  const packs=await currentContentPacks();
  const activeRulesetId=catalogState.status?.activeRulesetId||'all-provided-material';
  const rows=packs.map(p=>{
    const required=(p.requiredBy||[]).map(x=>x.name||x.id).filter(Boolean);
    const status=p.enabled?chip('ACTIVE','chip-green'):chip('DISABLED','chip-blue');
    const origin=p.imported||p.androidImported?chip('IMPORTED','chip-purple'):chip('BUNDLED','chip-neutral');
    const toggle=p.locked||p.toggleable===false?`<span class="mini-action disabled" title="Required core pack">Locked</span>`:`<button class="mini-action" onclick="closeModal();setAndroidContentPackEnabled('${p.id}',${p.enabled?'false':'true'})">${p.enabled?'Disable':'Enable'}</button>`;
    const uninstall=p.removable?`<button class="mini-action danger" onclick="closeModal();uninstallAndroidContentPack('${p.id}')">Uninstall</button>`:'';
    return `<div class="android-pack-row"><span><strong>${esc(p.name||p.id)}</strong><small>${esc(p.id)} · v${esc(p.version||'')}</small><small>${p.enabled?'Active in current Ruleset':'Disabled in current Ruleset'}${required.length?` · required by ${esc(required.join(', '))}`:''}</small></span><div class="android-pack-controls">${status}${origin}${toggle}${uninstall}</div></div>`;
  }).join('');
  modal(`<div class="row-between"><div><h2 style="margin:0">Content Packs</h2><p class="muted" style="margin:.25rem 0 0">Ruleset: ${esc(activeRulesetId)}</p></div></div><p class="muted">Disable removes a pack from the active Ruleset without deleting it. Imported packs can also be uninstalled. Bundled packs stay on the device but can be disabled, except the PTU Core pack.</p><label class="btn btn-success file-label full">＋ Import .ptucp<input type="file" accept=".ptucp,application/zip,application/octet-stream" onchange="closeModal();importContentPackFile(this.files[0])"/></label><div class="android-pack-list all-packs">${rows||'<p class="muted">No Content Packs found.</p>'}</div><div class="modal-actions"><button class="btn btn-ghost full" onclick="closeModal();openSaveTools()">← Save & Campaign Tools</button></div>`,{title:'Content Pack Manager',subtitle:'Enable, disable or uninstall campaign content.'});
}
...
async function openSaveTools(){
  let revisionHtml='<p class="muted">Android keeps local autosave data on this device.</p>';
  if(persistenceMode==='sqlite'){
    try{const response=await fetch('/api/revisions?limit=8',{cache:'no-store'});const payload=await response.json();revisionHtml=(payload.revisions||[]).length?`<div class="revision-list">${payload.revisions.map(r=>`<div><span><strong>#${r.id}</strong><small>${new Date(r.created_at).toLocaleString()}</small></span><button class="mini-action" onclick="closeModal();restoreRevision(${r.id})">Restore</button></div>`).join('')}</div>`:'<p class="muted">No revisions yet.</p>';}catch(error){revisionHtml='<p class="negative">Could not load revisions.</p>';}
  }
  const packHtml=window.PTU_ANDROID_BUILD?`<hr/><h3>Content Packs</h3><p class="muted">Open the dedicated Pack Manager to enable/disable bundled packs or uninstall packs imported from <code>.ptucp</code>.</p><button class="btn btn-ghost full" onclick="closeModal();openContentPackManager()">📦 Manage Content Packs</button>`:'';
  modal(`<h2>Save & Campaign Tools</h2><p><strong>Persistence:</strong> ${window.PTU_ANDROID_BUILD?'Android local autosave':persistenceLabel()}.</p><div class="modal-actions"><button class="btn btn-primary" onclick="exportSave();closeModal()">Export JSON Save</button><label class="btn btn-success file-label">Import JSON<input type="file" accept="application/json,.json" onchange="closeModal();importSaveFile(this.files[0])"/></label><button class="btn btn-danger" onclick="closeModal();resetState()">Reset Sample Campaign</button></div>${packHtml}<hr/><h3>Current save</h3><dl class="detail-dl"><div><dt>Trainer</dt><dd>${esc(trainer().name)}</dd></div><div><dt>Pokémon</dt><dd>${state.pokemon.length}</dd></div><div><dt>Rosters</dt><dd>${state.rosters.length}</dd></div><div><dt>NPCs</dt><dd>${state.npcs.length}</dd></div></dl><hr/><h3>Revisions</h3>${revisionHtml}`);
}
function openGlobalActions(){ const editorButton=window.PTU_ANDROID_BUILD?'':`<button class="btn btn-ghost" onclick="closeModal();route('editor')">Open Editors</button>`; const packButton=window.PTU_ANDROID_BUILD?`<button class="btn btn-ghost" onclick="closeModal();openContentPackManager()">📦 Content Packs</button>`:''; modal(`<h2>${window.PTU_ANDROID_BUILD?'App Settings':'Prototype Controls'}</h2><div class="modal-actions"><button class="btn btn-primary" onclick="closeModal();openSaveTools()">Save / Import / Export</button>${packButton}<button class="btn btn-ghost" onclick="closeModal();toggleGmOverride()">Toggle GM Override</button>${editorButton}</div><p class="muted">${window.PTU_ANDROID_BUILD?'Android player edition. Editors remain desktop-only; .ptucp packs can be imported and managed locally.':'Desktop beta controls.'}</p>`); }
