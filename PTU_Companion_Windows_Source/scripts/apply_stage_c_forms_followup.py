...
def patch_api(path:Path)->None:
    ...
    # Successful preview/reference responses must expose the resolution metadata used by Stage C.
    ...
    # Permanent progression/configuration uses the permanent Form, never an ephemeral transformation.
    ...
    # Evolution identity belongs to the canonical Species, even when its permanent Form overrides display fields.
    ...
    # Reference combat data uses resolved Form mechanics, but evolution lineage remains anchored to canonical Species identity.
    route='/api/pokemon/reference-data'
    for old,new in [
...
def patch_app(path:Path)->None:
    ...
    # Let the manager recover even when an already-stored Form has become invalid after a pack/rule/state change.
...
"""
    ...
    old="${warnings.length?`<div class=\"builder-validation bad\"><strong>Form warnings</strong><ul>${warnings.map(w=>`<li>${esc(w)}</li>`).join('')}</ul></div>`:''}<h3>Permanent / Base Form</h3>"
    new="${errors.length?`<div class=\"builder-validation bad\"><strong>Current Form needs attention</strong><ul>${errors.map(e=>`<li>${esc(e)}</li>`).join('')}</ul><small>You can use the choices below to recover to a valid Form state.</small></div>`:''}${warnings.length?`<div class=\"builder-validation bad\"><strong>Form warnings</strong><ul>${warnings.map(w=>`<li>${esc(w)}</li>`).join('')}</ul></div>`:''}<h3>Permanent / Base Form</h3>"
    if old in text:
        text=text.replace(old,new,1); changed=True
    elif new not in text: raise RuntimeError(f'Forms manager error display anchor missing in {path}')
    ...
    # Evolving to a different Species cannot retain a Form ID owned by the pre-evolution Species.
