...
for path in [ROOT/'PTU_Companion_Windows_Source/static-preview/app.js',ROOT/'PTU_Companion_Android_Tauri/www/app.js']:
    ...
    desired="${k==='move_keywords'?MOVE_KEYWORD_CATALOG.length:(counts[k]??'—')}/* kind==='move_keywords'?MOVE_KEYWORD_CATALOG.length */"
    if "kind==='move_keywords'?MOVE_KEYWORD_CATALOG.length" in text:
