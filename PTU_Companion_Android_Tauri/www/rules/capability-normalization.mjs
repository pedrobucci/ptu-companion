...
function terrainsFromNaturewalkName(name) {
  const match = String(name || '').trim().match(/^naturewalk\s*(?:\[([^\]]+)\]|\(([^)]+)\))$/i);
...
export function normalizeCapability(capability) {
  if (!isNaturewalkCapability(capability)) return capability && typeof capability === 'object' ? {...capability} : capability;
  const terrains = normalizeNaturewalkTerrains(capability);
  const source = capability && typeof capability === 'object' ? capability : {};
  return {...source,name:'Naturewalk',kind:source.kind || 'special',capability_id:source.capability_id || 'naturewalk',terrain:terrains,terrains};
}
